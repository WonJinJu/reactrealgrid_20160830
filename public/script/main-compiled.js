...
function setGridCallbackFunc_Right() {
    ...
    gridView_Right.onCellEdited = function (grid, itemIndex, dataRow, field) {
        ...
        if (focusCell.fieldName == "nm_deptemp" || focusCell.fieldName == "yn_disabled") dicEditCell[focusCell.fieldName] = gridView.getValues(itemIndex)[focusCell.fieldName];else dicEditCell_Right[focusCell.fieldName] = gridView_Right.getValues(itemIndex)[focusCell.fieldName];
        //focusCell.dataRow = 0;
        //alert(focusCell.dataRow)
...
function setProviderCallbackFunc_Right() {
    ...
    dataProvider_Right.onRowUpdated = function (provider, row) {
        ...
        var cdEmp = getCdEmp();
        if (Object.keys(dicEditCell).length > 0) callUpdateAPI(cdEmp);
        if (Object.keys(dicEditCell_Right).length > 0) callUpdateAPI_Right(cdEmp);
    };
}
...
function callUpdateAPI_Right() {
    ...
    if (cdDept.length == 2 && cdEmp.length == 2 && dicEditCell_Right != undefined && Object.keys(dicEditCell_Right).length > 0) {
        ...
        var param = [];
        for (var item in dicEditCell_Right) {
